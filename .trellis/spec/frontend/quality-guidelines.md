...
`src/assets/kaf/generated/mediaVariants.ts` is also excluded from Prettier. Its
exact bytes are generated and verified by
`scripts/generate_kaf_media_variants.py`; formatting it independently would
make the generated TypeScript drift from the manifest verifier.
...
release before Works, six-image reduced-motion flow, the complete five-album sequence and
verified `狂想β` cover, the
